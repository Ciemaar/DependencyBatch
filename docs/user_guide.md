...
class MyAnalysisJob(Job):
    ...
    def store_results(self, archive_path: Path) -> None:
        # Implement logic to save the results tarball somewhere
...
The `Job` class includes `depends_on` and `result_of` attributes (sets of `Job` objects). While the base `Queue` implementations do not automatically resolve dependency graphs, you can use these attributes to build your own task orchestration logic.

### Results

You can add file paths (`pathlib.Path` objects) generated by your job to the `self.results` set.

When the job is closed (either by explicitly calling `.close()` or when the context manager exits), the base library automatically calls `handle_results()`. This method compresses all the files in `self.results` into a temporary tarball and then calls the abstract method `store_results(archive_path)`.

Any subclass of `Job` must implement `store_results()` to dictate what happens to that final archive (e.g., upload it to a cloud bucket or move it to a persistent directory).
